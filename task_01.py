...
def fitness_function():
    ...
    for i in range(popsize):
        ...
        ff_obstacle.append(
            1 - ((obstacles[i]-min(obstacles))/(max(obstacles)-min(obstacles))))
        ff_turn.append(1 - ((Turns[i]-min(Turns))/(max(Turns)-min(Turns))))
        ff_path.append(
            1 - ((no_of_steps[i]-min(no_of_steps))/(max(no_of_steps)-min(no_of_steps))))
        final_fitness.append((100*w_obs*ff_obstacle[i]) * (
            ((w_path * ff_path[i]) + (w_turn * ff_turn[i])) / (w_path + w_turn)))
